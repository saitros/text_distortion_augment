...
class AugModel(nn.Module):
    ...
    def forward(self, src_input_ids, src_attention_mask, src_token_type_ids):
        ...
        # Sampling Z
        z = self.sample_z(encoder_out, sigma=self.z_variation)
        ...
        decoder_out = self.decoder_norm(self.decoder_linear2(decoder_out))

        return encoder_out, decoder_out, z

    def sample_z(template: torch.tensor = None, sigma: float = 2):
        return sigma * template.data.new(template.size()).normal_()

    def generate(self, src_input_ids, src_attention_mask, src_token_type_ids):
        # Encoding
        encoder_out = self.model(input_ids=src_input_ids, attention_mask=src_attention_mask,
                                 token_type_ids=src_token_type_ids)
        encoder_out = encoder_out['last_hidden_state'].mean(dim=1)
